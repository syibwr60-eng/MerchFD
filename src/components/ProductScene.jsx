...
export function ProductScene({ product, index, align = 'left' }) {
  ...
  const printY = useTransform(scrollYProgress, [0, 1], ['8%', '-8%'])
  const textY = useTransform(scrollYProgress, [0, 1], ['0%', '-15%'])
  const imageScale = useTransform(scrollYProgress, [0, 0.5, 1], [1.08, 1, 0.94])
  const isRight = align === 'right'

  return (
    <section
      ref={sectionRef}
      className="relative min-h-[110svh] overflow-hidden px-4 py-8 sm:px-8 sm:py-12 lg:min-h-[120svh] lg:px-12"
    >
      <motion.img
        src={product.print}
        alt=""
        ...
        animate={isVisible ? { opacity: 0.5, scale: 1, filter: 'blur(0px)' } : {}}
        transition={{ duration: 1.1, delay: 0.15, ease: [0.16, 1, 0.3, 1] }}
        className={`pointer-events-none absolute top-[4%] z-0 w-[100vw] object-contain mix-blend-screen lg:hidden ${
          isRight ? 'left-[-12vw]' : 'right-[-12vw]'
        }`}
      />
      <motion.img
        src={product.print}
        ...
        animate={isVisible ? { opacity: 0.34, scale: 1, filter: 'blur(0px)' } : {}}
      ...
      />
      <div className="pointer-events-none absolute inset-0 z-0 bg-[linear-gradient(180deg,rgba(10,10,10,0.5),rgba(10,10,10,0.18),rgba(10,10,10,0.92))] lg:bg-[linear-gradient(90deg,rgba(10,10,10,0.88),rgba(10,10,10,0.4),rgba(10,10,10,0.88))]" />
      <div className="pointer-events-none absolute inset-0 z-0 bg-[radial-gradient(circle_at_center,rgba(255,122,0,0.12),transparent_52%)]" />
      <div className="flex min-h-screen items-center lg:sticky lg:top-0">
        <div
          ref={revealRef}
          className={`relative z-10 grid w-full items-end gap-5 pt-[26vh] sm:gap-8 sm:pt-[28vh] lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:pt-0 ${
        ...
        >
          <motion.div
            ...
            whileHover="hover"
            className="group relative overflow-hidden rounded-[1.6rem] border border-white/10 bg-white/[0.03] p-3 shadow-glow sm:rounded-[2rem] sm:p-4"
          >
            <motion.div
              variants={cardMotion}
              initial="rest"
          ...
          <motion.div
            ...
            className={`relative rounded-[1.5rem] border border-white/8 bg-black/30 p-5 backdrop-blur-md sm:p-6 lg:border-none lg:bg-transparent lg:p-0 lg:backdrop-blur-0 ${isRight ? 'lg:pr-8' : 'lg:pl-8'}`}
          >
            ...
            <h3 className="max-w-xl text-[15vw] font-semibold uppercase leading-[0.84] tracking-poster text-white sm:text-[11vw] lg:text-[6vw]">
              {product.name}
            </h3>
            ...
            <p className="mt-4 max-w-lg text-[14px] leading-relaxed text-zinc-200 sm:hidden">
